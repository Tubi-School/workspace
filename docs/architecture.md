...
**`prisma generate` must work without a database.** CI type-checks and builds before any database exists, so `prisma.config.ts` falls back to a placeholder URL when `DATABASE_URL` is unset. Commands that genuinely connect — `migrate`, `studio` — then fail loudly, which is the correct outcome.
